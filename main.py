...
def get_args_parser():
    ...
    parser.add_argument('--skip_encoder', default=True, action='store_true')
    ...
    parser.add_argument('--region_prompt', action='store_true', default=True)
    ...
    parser.add_argument('--roi_feat', default='layer3', choices=['layer4', 'layer3'])
    ...
    parser.add_argument('--smca', default=True, action='store_true')
    ...
    parser.add_argument('--cls_loss_coef', default=3.0, type=float)
    ...
    parser.add_argument('--coco_path', type=str, default='/home/cccc/0-Done/CORA-region/data/coco')
    ...
    parser.add_argument('--region_prompt_path', default='/home/cccc/0-Done/CORA-region/logs/region_prompt_R50.pth', type=str)
    parser.add_argument('--visual_prompt_path', default='/home/cccc/0-Done/CORA-region/logs/checkpoint.pth', type=str)
    parser.add_argument('--batch_size', default=8, type=int)
    parser.add_argument('--output_dir', default='/home/cccc/0-Done/CORA-region/logs/group_cls1', help='path where to save, empty for no saving')
    parser.add_argument('--visual_dir', default='visual_group', help='path where to save, empty for no saving')
    parser.add_argument('--device', default='cuda:0', help='device to use for training / testing. We must use cuda.')
    parser.add_argument('--seed', default=42, type=int)
    # parser.add_argument('--resume', default='/home/cccc/0-Done/CORA-region/logs/group/checkpoint.pth', help='resume from checkpoint, empty for training from scratch')
...
def main(args):
    ...
    if args.region_prompt:
        # prompt_names.append('region_pertur.patch')
        prompt_names.append('visual_prompt.prompt')
        prompt_names.append('visual_prompt.prompt_key')
        prompt_names.append('mlp_inter')
        prompt_names.append('norm_post_tokens')
        prompt_names.append('pre_assign_attn')
        prompt_names.append('assign')
        prompt_names.append('norm_new_x')
        prompt_names.append('mlp_channels')
        for k, v in model.named_parameters():
            if match_keywords(k, prompt_names):
                # print(k)
                v.requires_grad = True
    # model.backbone[0].attnpool.positional_embedding.requires_grad = False
